...
public class ownerTasks {
	...
	public ModelAndView getreq(@ModelAttribute taskVo taskvo,HttpServletRequest req,ownerCompanyVo companyVo,ownerContactVo contactVo )
	{
	...
	public ModelAndView save(@ModelAttribute taskVo taskvo,HttpServletRequest req,ownerContactVo ownerContactVo,@RequestParam CommonsMultipartFile file,HttpSession session,documentVo vo) throws Exception
	{	
		String userName = BaseMethod.getCurrentUser().getUsername(); 
		loginVo loginVo = new loginVo();
		loginVo.setUserName(userName);
		...
		/*int contactid = Integer.parseInt(req.getParameter("ownerContactVo.contactid"));
		ownerContactVo ownerContactVo = new ownerContactVo();
		ownerContactVo.setContactid(contactid);
		ownerContactVo.setLoginVo(loginVo);
		taskvo.setOwnerContactVo(ownerContactVo);
		Date date = new Date();
		taskvo.setTaskDate(date);*/
		...
		try {
			credentials = new BasicAWSCredentials("AKIAJHII3MOGLLFOJM6Q", "iaEKsMi4ovs2g/dUJcNYk7Pp/X0nrPUn2naHnBDa");
...
*/		PutObjectRequest objectRequest = new PutObjectRequest(bucketName, userName+"/Tasks/"+file.getOriginalFilename(), file.getInputStream() , null);
	    ...
	    BufferedOutputStream bout=new BufferedOutputStream(  
	             new FileOutputStream(path+"/doc/"+filename));  
	    bout.write(barr);  
	    bout.flush();  
	    bout.close();  
