...
public class owner_index {
	...
	public ModelAndView getreq(HttpServletRequest req,dealVo dealVo,ownerContactVo contactVo,taskVo taskVo,ownerCompanyVo companyVo,documentVo documentVo)
	{
	   ...
	   /* model.addObject("documents",docls.size());*/
		...
		/*for (Object[] dealVo : countDeal) {
			Integer month = dealVo.getDealDate().getMonth();
			map.put(month+1, countDeal.size());
		}*/
		...
		/*for (Iterator iterator = countDeal.iterator(); iterator.hasNext();) {
			...
			System.out.println("lllllllllll"+count[1]);
			map.put(count[0].toString(), count[1].toString());
